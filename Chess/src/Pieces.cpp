...
// Helper function: Checks if the squares between source and destination are empty.
...
// Factory: Creates a new Piece object based on the given character.
// Lowercase indicates White; uppercase indicates Black.
Piece* createPiece(char piece) {
    bool isWhite = (piece >= 'a' && piece <= 'z'); // White if lowercase.
    char p = toupper(piece); // Use uppercase for type decision.
    switch (p) {
